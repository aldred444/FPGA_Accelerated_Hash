module tle
 (
    // ---- FPGA-side I/O (these you assign pins for manually) ----
    input         FPGA_CLK_50,      // 50 MHz clock to fabric
    output  [7:0] LED,

    // ---- HPS DDR3 (pass-through; pinned by the DDR tcl script) ----
    ...
    // ---- HPS peripheral I/O (pass-through; pinned by tcl) ----
    ...
    wire [8:0] sum = operand1 + operand2;   // 9 bits: 255+255=510 needs bit 8
	 ...
	 assign LED[7]   = blink_ctr[24];   // ~1.5 Hz blink at 50 MHz
	 assign LED[6:0] = sum[6:0];
	 
    soc_system u0 (
        .clk_clk                                 (FPGA_CLK_50),
        .clk_130_clk                             (FPGA_CLK_50),   // see note below
        .reset_reset_n                           (hps_fpga_reset_n),

        // FPGA-fabric reset requests into HPS — tie inactive (active-low, so high)
        ...
        .hps_0_h2f_reset_reset_n                 (hps_fpga_reset_n),               // HPS->FPGA reset out, unused here

        // ---- operands in, result out (your first-contact signals) ----
        .operand1_pio_external_connection_export (operand1),
        .operand2_pio_external_connection_export (operand2),
        .result_pio_external_connection_export   (sum),            // 9-bit, matches result_pio width

        // ---- HPS I/O pass-through (name-for-name to top-level ports) ----
